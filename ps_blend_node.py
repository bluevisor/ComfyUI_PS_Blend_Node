...
#             c = torch.where(n < 0, l + (((c - l) * l) / (l - n)), c)
#             c = torch.where(x > 1, l + (((c - l) * (1 - l)) / (x - l)), c)
#             return c

#         def sat(c):
#             return torch.max(c, dim=-1)[0] - torch.min(c, dim=-1)[0]

#         def set_sat(c, s):
#             sorted_c, _ = torch.sort(c, dim=-1)
#             c_min, c_mid, c_max = sorted_c[..., 0], sorted_c[..., 1], sorted_c[..., 2]

#             delta = c_max - c_min
#             c_mid = torch.where(delta > 0,
#                                 ((c_mid - c_min) * s) / delta,
#                                 torch.zeros_like(c_mid))
#             c_max = s
#             c_min = torch.zeros_like(c_min)

#             return torch.stack([c_min, c_mid, c_max], dim=-1)
...
#         #     result = torch.where(img2 > 0, 1 - torch.min(torch.ones_like(img1), (1 - img1) / (img2 + 1e-8)), torch.zeros_like(img1))
#         elif blend_mode == "Color Burn":
#             # Ensure img1 and img2 are in the range [0, 1] to avoid overflow
#             img1 = torch.clamp(img1, 0, 1)
#             img2 = torch.clamp(img2, 0, 1)
#             result = 1 - torch.clamp((1 - img1) / (img2 + 1e-8), 0, 1)
import torch
import numpy as np
